...
async function startServer() {
  ...
  // Support large base64 file payloads for batch multimodal processing
  ...
  // Multimodal Gemini Document Classification & Renaming Endpoint
  app.post('/api/classify', async (req, res) => {
    try {
      const { folderName, filesData } = req.body;
      if (!filesData || !Array.isArray(filesData) || filesData.length === 0) {
        return res.status(400).json({ error: 'filesData array is required and must not be empty' });
      }

      const apiKey = process.env.GEMINI_API_KEY;
      if (!apiKey) {
        return res.status(500).json({
          error: 'GEMINI_API_KEY environment variable is not configured on the server. Please set GEMINI_API_KEY in your settings or environment.'
        });
      }
...
`;
      ...
      filesData.forEach((fileObj: any, idx: number) => {
        parts.push({
          text: `\n--- FILE #${idx + 1} | ID: "${fileObj.item?.Id || fileObj.item?.id}" | Original Filename: "${fileObj.item?.FileName || fileObj.item?.Name || fileObj.item?.name}" ---`
        });
        if (fileObj.base64) {
          parts.push({
            inlineData: {
